...
The algorithm using bisimiluation up to congruence for checking inclusion of languages of NFA is described in its own section (\ref{sectCongr}). This algorithm
reduces number of macro states that are neccessary to check to determine if inclusion holds or not thanks to the computation of a congruence closure of a relation
...
\begin{center}
$X\rightarrow X+Y$ \hspace{5cm} $Y\rightarrow X+Y$
\end{center}
...
An example (taken from \cite{popl13}) of applying this approach for checking inclusion of NFA $\mathcal{A}$ and $\mathcal{B}$ (both are on
figure \ref{example}) is given on the figure \ref{example}.
