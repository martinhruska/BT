...
in chapter \ref{implementation}. The evaluation of the optimized algorithms for the inclusion checking is in chapter \ref{eval}. The summarization of the whole
...
\end{center}
  ...
  %TODO nejak inteligentne to zaradit
		...
		\begin{itemize}
			...
			\item There is maximal one nonterminating state, which can always make a transition only to itself.
...
it starts building not only $\mathcal{B}_{det}$ but also $\mathcal{A}_{det}$ because the purpose of this algorithm is check language equivalence so it builds a
product automaton $\mathcal{A}_{det} \times \mathcal{B}_{det}$ which states (so-called product states) are the pairs 
...
The algorithm searchs for a victim that proves $L_\mathcal{A} \neq L_\mathcal{B}$. The victim is a product state $(P_\mathcal{A},P_\mathcal{B})$ 
which breaks a condition that
the $P_\mathcal{A}$ contains a final state of $Q_\mathcal{A}$ if and only if $P_\mathcal{B}$ contains a final state of $Q_\mathcal{B}$. 
...
usage of rewriting rules of the relation $R$ is denoted as $X{\downarrow_R}$.
...
A derivation of both normal forms is shown on \ref{figHKCRew}. The result of the derivation is 
that the normal set of $\{x,y\}$ is a set $\{x,y,z,u\}$ which is same as the derived normal form of $\{u\}$. It confirms that $(\{x,y\},\{u\})\in c(R)$ and
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{The figure (taken from \cite{popl13}) shows the deriving of the normal forms of the sets $\{x,y\}$ and $\{u\}$ using rewriting
      ...
      $\{u\}\rightarrow\{y,z,u\}$ (based on product state $(\{y,z\},\{u\})\in R$), so the result is $\{x,y,z,u\}$. The normal form of the set $\{u\}$
...
Due this conditions the time complexity for finding one rule is in the worst case $rn$, where $r=|R|$ and $n=Q$ where $Q$ is set of states of an NFA.
...
checking inclusion. An optimization is possible also in checking whether macrostate $(X,Y)$ is in congruence closure of a relation $R$ of the visited
...
\mathcal{B}$ so in all macrostates $(X,Y)$ is $X$ set of states of NFA $\mathcal{A}\cup\mathcal{B}$ and $Y$ set of states of NFA $\mathcal{B}$. Since the
states of \mathcal{B} are allready in macrostate $X$ it is useful to use the rewriting rules only in following form \cite{popl13}:
\begin{center}
...
the set $\{x,y,u\}$ is subset of the derived set so it holds that $(\{x,y,u\},\{u\})\in c(R)$.
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{The figure shows the deriving of the normal forms of the sets $\{x,y,u\}$ and $\{u\}$ using rewriting
      ...
      $\{u\}\rightarrow\{y,z,u\}$ (based on $(\{y,z,u\},u)$) so the derived normal form is set $(\{x,y,z,u\})$. 
...
Library can use as input regular expression created by the Java \emph{RegExp} class.
...
The algorithms using the antichains are possible to use not only for finite automata but also for tree automata(\cite{cav06,taca10}). 
...
no library or toolkit simile to VATA library providing these algorithms for language inclusion checking over NFA.
...
Purposes of VATA library are similar as purposes of this work and because VATA also provides basic infrastructure for 
parsing, serializing and writing finite automata from an input format, 
...
of transition of given automaton, its final states etc.). The main program choose one of the internal encodings of the automata. These encodings
contain a definition of a data structure for a representation of automaton and functions which create transform the automaton from the data structure
...
Finally, once all needed operations over the input automata are processed, the result automaton can be dump to a output text format using one of the serializers 
(currently there is implemented only the Timbuk format serializer too).
...
automata will be definitely more efficient. Not only inclusion checking algorithm will be implemented but also the algorithms for basic operations like
union, intersection, removing unreachable or useless states etc.
...
between a start or a final state and the other states. But it is not necessary to store the whole set of states because states that are not start or final
...
one often wants to get all transitions for given state or for given state and given alphabet symbol. The similar situation is in the case
of tree automata when it is not necessary to hold the whole set of state but it is important to have an efficient data structure for representing 
transitions of the given tree automata.

The data structure used for storing transitions of a tree automaton in the VATA library was described earlier \ref{sectionExplicitEnc} and can be seen
on the figure \ref{figExplicitTreeDataStr}. The evaluation of the VATA library \cite{libvata} proves efficiency of this data structure is efficient, so it was
decided to modify this data structure and use it also for part of the VATA library for finite automata.
...
The data structure for storing of the finite automata transitions is simplification of the data structure for the tree automata because a tree automaton's
transition has following form: $(q_1,\ldots,q_n) \xrightarrow{a} q$ where $q,q_1\ldots q_n$ are states of the tree automaton and $a$ is the symbol of 
alphabet of the tree automaton, while a finite automaton has transition in form: $q_1 \xrightarrow{a} q_2$ where $q_1,q_2$ are states of the finite automaton and
$a$ is its symbol so the left side of the transition is simpler in the case of finite automaton. A comparison of data structure for the finite 
automata and the tree automata can be seen by comparing figures 
\ref{figExplicitFADataStr} and \ref{figExplicitTreeDataStr}. This simplification is possible because the tree version has to stored to whole tuples of
states in transitions. Since these tuple can be very large it is more efficient to store them only once and in data structure keeps pointer to the 
tuple, which is in a transition, instead of the tuple alone. 
...
bring any memory efficiency (the size of a pointer to a state and the state alone is quite similar). This fact causes that in the data structure 
...
set of states would be pointed from transition cluster and would contain all states accessible from a given state under a certain symbol of the input alphabet.

But there is possible another simplification. The set of states does not need to be in special level of data structure but can be integrated to the transition
...
The mentioned optimization enables that the tree version of data structure, which has four levels, was simplified to the two level data structure what brings
simpler and more efficient manipulation with these data structure.

This data structure apply also the copy-on-write principle, what brings better memory efficiency. It means that the look-up tables and 
...
As it was mentioned before (section \ref{analysis}) it is necessary to keep start and final states in the special sets to 
...
over finite automata so there is no need to create special data structure and unordered set is efficient enough for them.

\section{Translation of the states and symbols}
\label{sectionTranslate}
An input automaton is always parsed and converted to the intern representation from its input format. 
During this conversion to the intern representation are states translated (mapped) from input type (e.g. text description of the automaton) 
to the integers and this principle is also applied for symbols of the input alphabet (principle can be seen on figure \ref{figExplicitFADataStr}). 
...
is changed the old value is mapped to the new one in a hash table what helps to keep mapping of the original input to the new value.

When the operations over a NFA are performed the NFA is often serialized back to the input format. The result 
automaton's states are converted back to the input format by using hash tables where is mapping to the internal representation stored. This
principle leads to the better readable output after serialization because notation is not changed.

\begin{figure}
\begin{center}
\input{fig/translator.tikz}
		\label{figExplicitFADataStr}
    \caption{The figure show principle of translation of the input format to the internal representation which is done by the hash table like those
      on the figure. The states (the left hash table) or the symbols (the right hash table) of an NFA are strings in the input format but are mapped to integers
    which are used as the internal representation of these states and symbols.}
\end{center}
\end{figure}

\section{Usage of the Timbuk format}
\label{usageTimub}
VATA library provides command-line interface and it is possible to load a finite automaton from a text specification. 
The text specification has to have some standard format but there is no such format for the finite automata so it was chosen
...
the union more readable because the states have the same names as it has in the input automata, only indices $1$ and $2$ are added for 
...
The algorithm for intersection of 
is described by algorithm \ref{algIntersection}.

The principle of this algorithm is following. 
The both NFA are explored parallel and to the result automata are added just product states consisting two states (each from different automaton) 
...
reachable from this start states for some word $w \in \Sigma^{*}$. So firstly, all reachable states are found and added to a set of the reachable states. 
...
Get a candidate (word), also called get a witness, is operation over an NFA $\mathcal{A}$ which creates an NFA $\mathcal{B}$ which language $L(\mathcal{B})$
...
states accessible from current set of reachable states are added to this set. This is repeated until the whole NFA $\mathcal{A}$ is copied to the NFA 
...
The parser returns a data structure which generally describes a finite automaton and this data structure is further processed in part of library for 
...
of an finite automaton to the explicit encoding, the class \emph{Antichain2Cv2} for representing of antichain during checking inclusion using
antichain algorithm \ref{sectionAntichain} and the class \emph{AutDescription} for representing an automaton after the parsing.

The usage of the of this utilities speeds up the development of the module for finite automata and also keeps the library more compact because no
redundant code is produced.

\section{Macrostate Cache}
\label{sectionCache}
In both mentioned algorithms (\ref{sectionAntichain},\ref{sectionCongr}) for checking inclusion of NFA are compared set of states, 
respectively is checked some relation between this these some sets of states. It is possible that there will be needed to check some relation between
...
When the $(p_1,P)\sqsubseteq(q_2,Q)$ is being checked the relation between $p_1$ and $q_2$ is very easy to get between they are just two states, but checking
...
be created. After the transitions are processed, there is added other one partition which represents start state. Then the simulation relation is initialized
...
of NFA \ref{subsectionCongr} and its optimized version for checking inclusion was implemented too \ref{congrOpt} 
...
This technique reduces the number of the states of $\mathcal{A}\times\mathcal{B}$ for which is necessary compute congruence closure what helps improvement in
...
This simplification leads to the implementation optimization, which is based on creating of $X{\downarrow_R}$ which has as much state as possible and all
...
This optimization is further enhanced by the improvements in implementation. Once there is checked a generated state $(X,Y)$ and the normal form 
$Y{\downarrow_R}$ is computed it is efficient to store all rewriting rules that was used during the computation because there can be
...
which contains this macrostate. Notice that is not able to store elements of $R$ which rewriting rule is not applicable because there are added gradually
new elements to $R$ and after applying of the rewriting rules of these elements can be also usable rule which has not been usable before.%Preformulovat
...
normal forms of macrostates which has not been already explored. Desperation of these two situation prevents slowing down of algorithm by trying applying
...
For both of the evaluations were used NFA for model checking provided by Dr. Lukáš Holík.
...
The comparison of the VATA library implementation and OCaml implementation can be seen on the figure \ref{figGraphOCaml}. The plot shows the relation between 
...
\begin{center}
  \begin{tabular}{ | l | r | r |}
   ...
   \end{tabular}
   \caption{Table gives summarizing of the evaluation and gives the two information. The first one is
...
\subsection{Comparison with VATA library Tree Automata Implementation}
...
is given on the figures \ref{figPlotAc} and \ref{figPlotCa}, particular on the left plot in these figures.
...
\label{figPlotAc}
\caption{On the both plots
  is comparison of VATA library implementation of antichain algorithm for tree automata in upward direction (red points) 
...
\parbox{.45\linewidth}{
  ...
  \begin{tabular}{ | l | r | r |}
   ...
   \caption{The table gives information in how many case was faster on implementation over the other and how many times was the better implementation
     averagely faster. The left table shows comparison of VATA library for tree automata with checking inclusion upward with antichain algorithm with
...
\caption{The figure shows
 comparison of VATA library implementation of antichain algorithm for tree automata in downward direction using cache optimization (red points) 
   ...
   of the test has been processed.}
\end{center}
\end{figure}
...
The new extension of VATA library uses explicit encoding for representation of the finite automata. The data structures for the encoding has been designed
...
To achieve the best performance for language inclusion checking are used the state-of-the-art algorithms, so-called antichains and so-called bisimulation
...
The bisimulation up to congruence algorithm is implemented its general version (for checking equivalence of finite automata) and also in
...
The implementation of the antichains algorithm can be optimized by avoiding repetition of some computation. For the bisimulation up to congruence
...
fully integrated to the new extension and its algorithms to be able to use it with all its advantages. The full integration of the new extension could be done
