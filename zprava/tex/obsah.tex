...
The goal of this work is two-fold: (i) extending the VATA library with an NFA module implementing basic operations on NFA, such as union, intersection, or 
...
and $P$ is not final in automaton $\mathcal{B}_{det}$. %TODO zamyslet se nad definici final makrostavu
...
\begin{algorithm}[H]
	...
	\KwOut{TRUE if $\mathcal{L_\mathcal{A}}\subseteq\mathcal{L_\mathcal{B}}$. Otherwise FALSE.}
		\If{there is a witness product-state in \{$(i,I_{\mathcal{B}})\,|\,i\in I_{\mathcal{A}}$\}}
			{return FALSE\;}
		$Processed$:=$\emptyset$\;
    %$Next$:= Initialize(\{$(s,Minimize(I_B))\mid s\in I_A$\})\;
		...
		\While{$Next\neq \emptyset$}
		{
			...
			{
				\eIf{$(p,P)$ is a witness product-state}
				{
					\Return FALSE\;}
					{
						\If{$\nexists p'\in P\ s.t.\ p\preceq p'$}
						{
							\If{$\nexists (x,X) \in Processed\cup Next\ s.t.\ p\preceq x \wedge X\preceq^{\forall \exists}P$}
							{
...
We define tree automata and related terms before the description of the VATA library itself. The \emph{ranked} alphabet is a finite set of symbols
...
to extend it by another module. It was therefore decided not to create a brand new library but implement new extension of VATA for finite automata in C++
language.
...
\end{center}
\end{figure}
On the first line in Figure \ref{figTimbukFormat} 
...
\begin{algorithm}[H]
  ...
  \While{$Stack\neq \emptyset$} {
        ...
        Add $\{(q\xrightarrow{a}q')\ |\ q'\in Q_\mathcal{A}\,.\,\exists a \in \Sigma\,.\,q\xrightarrow{a}q'\in \delta_\mathcal{A}\}$ to $\delta_\mathcal{B}$\;
        \If{$(q \not\in\ Reachable)$} {
          Push $q$ on $Stack$\;
          Add $q$ to $Reachable$\;
        }
...
\begin{algorithm}[H]
  ...
  \While{$Stack\neq \emptyset$} {
			Pick and remove a state $p$ from $Stack$\;
			\ForEach{$p\xrightarrow{a}q\in\{p\xrightarrow{a}q'\ |\ q'\in Q_\mathcal{A}\,.\,\exists a\in \Sigma\,.\,p\xrightarrow{a}q'\in\delta_\mathcal{A}\}$}{
        \If{$(q \not\in\ Reachable)$} {
          Push $q$ on $Stack$\;
          Add $q$ to $Reachable$\;
        }
...
\caption{The figure shows a
 comparison of the VATA library implementation of the anti\-chains algorithm for tree automata in the downward direction using the cache optimization 
 with the VATA library implementation of the congruence algorithm for NFA.}
\label{fig:figPlotCa}
\end{center}
\end{figure}
