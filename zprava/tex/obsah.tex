...
derivation of the normal form of the set $\{u\}$ is shown on the figure \ref{figHKCRewO}. The result of this derivation is a set $\{x,y,z,u\}$ and because
the set $\{x,y,u\}$ is subset of the derived set it holds that $(\{x,y,u\},\{u\})\in c(R)$.
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{The figure shows the deriving of the normal form the set $\{u\}$ using rewriting
      rules of the elements of a relation $R=\{(\{x,u\},\{u\}),(\{y,z,u\},\{u\})\}$. The normal form of the set $\{u\}$
...
VATA library for manipulating of \emph{tree} automata will be introduced. It will be briefly described library design, operations for tree
...
The purposes of VATA library are similar as purposes of this work and because VATA is written in modular way it is easy to extend it by another module so it
was it was decided not to create a brand new library, but implement a new extension of VATA for finite automata.
...
is implemented only Timbuk format parser). A result of parsing is a data structure with the information about automaton (the data structure stores a list
of transitions of a given automaton, its final states etc.). The main program choose one of the internal encodings of the automata. The encodings
differs by a data structure they use for a representation of automaton. Each encoding also provides the functions for transformation of the automaton 
...
When the automaton is processed it is often dumped to output format. 
...
union, intersection, removing unreachable or useless states.
