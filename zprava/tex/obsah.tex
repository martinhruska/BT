...
\input{fig/tacas12/explicit.tikz}
    ...
    \caption{The data structure for storing transtion of an tree automaton. There is a hash table (so-called top-level lookup table) 
      for a given state which map this state to the pointer to another hash table (so-called transition cluster). Transition cluster
      maps a symbols to the pointer to the set of pointers to tuples.}
\end{center}
\end{figure}
...
Data structures, which have been used for representation of
...
The set of transitions keeps the most information about an NFA and is often used
during operations over NFA, so the the performance of these operations hardly depend on efficiency of data structure for set of transitions. For example,
one often has an state of an NFA and wants to get all states accessible for this state for each input alphabet symbols. The similiar situanion is in the case
...
on the figure \ref{figExplicitTreeDataStr}. The evaluation of VATA library \cite{libvata} shows that this data structure is efficient enough, so it was
...
popsat strukturu samotnou
obrazek
porovna s orignialnim

\section{Translation of the states and symbols}
popsat, ze taky predkladam do integre a ze jsem to vzla z vaty a ze je to efektivnejsi
\section{Overview of used interfaces of VATA library}
popsat, co jsem se rozhodl pouzit z vaty a proc
