...
\section{Regular Languages}
		A language $L$ is \emph{regular}, if there exists an NFA $\mathcal{A}=(Q,\Sigma,\delta,I,F)$, such that $L=L(\mathcal{A})$.
...
the \emph{language inclusion problem} is decision whether $L(\mathcal{A}) \subseteq L(\mathcal{B})$ what is defined by standard set operations as
$L(\mathcal{A})\cap \overline{L(\mathcal{B})} = \varnothing$.
This problem is PSPACE-complete \cite{cav06}. For decision of language inclusion problem there exists some classical algorithms.
Classical algorithms are based on searching of accepting state of the product automaton $A\times \overline{B}$. Such state can serve like \emph{witness}, that
