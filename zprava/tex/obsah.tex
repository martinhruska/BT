...
\begin{figure}[h]
	\begin{center}
		\scalebox{0.6}
		{
			...
			\includegraphics{fig/tacas12/explicit.tikz}
		}
		\caption{The VATA library design. The image is taken from \cite{libvata}}.
		\label{picVataDesign} %TODO podivat se na font
	\end{center}
\end{figure}
\end{comment}
...
\chapter{Design}
\label{arch}
In this chapter will be described design of the newly created extension of VATA library. Firstly, data structures, which have been used for representation of
finite automata in the explicit encoding, will be described. These data structures was created by simplifying the data structure for representing tree automata
in explicit encoding so the comparsion of both will be given.
%TODO dodelat podle konecneho obsahu

\section{Data Structure for Explicit Encoding of Finite Automata}
\section{Analysis}
An NFA is defined by set of its states, its start and final states (which are subset of all states of a NFA) and also its
transitions \ref{defNFA} and the input alphabet. But not all of the parts of the definition have to be saved alone. The set of states will be
implicitly given by transitions and there is no need to save them alone in some special set. The same fact holds for input alphabet. On the other hand, 
it is neccessesary to create special data structure for start state and final state to be able to distinguish between start state or final state 
and ordinary state of an NFA, what is not possible to recoqnize from transtion alone. 

While a data structure for storing the final and start states can be in set, data structure for representing the transtions of an NFA is more complicated.

For the explicit encoding of tree automata is used data structure described earlier in this documents \ref{sectionExplicitEnc}. The evaluation of performance of 
VATA library was proven that this data structure works quite well.
\subsection{Symplifying of the original data structure}
\subsection{Keeping the start states}
\subsection{Translation of the states and symbols}
\section{Overview of used interfaces of VATA library}
\section{Usage of timubk format}
