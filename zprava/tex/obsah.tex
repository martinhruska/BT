...
%TODO dodelat
First, the language will be defined, then finite automata and their context, the regular language and its closure properties. 
\label{teorie}
...
\section{Regular Languages}
    ...
    Regular languages are closed under certain operation, if result of this operation on some regular language is always regular language too.
		...
		\begin{itemize}
			...
			\item Reversal: $\{a_1\dots a_n \in L \ |\ y=a_n\dots a_1\}$
			\item Iteration $L^{*}$ %: $L^{*}$
			\item Concatenation: $L\ldot K=\{x\ldot y\ |\ x\in L \wedge y\in K\}$
			\end{itemize}

\begin{comment} Presunout nebo zahodit
