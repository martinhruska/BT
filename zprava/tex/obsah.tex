...
so it holds that $\Sigma^{*}=\Sigma_{+}\cup\epsilon$. A~\emph{language} $L$ over $\Sigma$ is a subset of $\Sigma^{*}$.
...
If the generated pair is in this congruence closure, it can be skipped and further not processed.
	...
	\begin{algorithm}[h]
		...
		\KwOut{TRUE, if $L(A)$ and $L(B)$ are in equivalence relation. Otherwise, FALSE.}
			$Processed = \varnothing$\;
			$Next = \varnothing$\;
			insert($I_A$,$I_B$) into $Next$\;
			\While{$Next \neq \varnothing$}
			{
				extract $(X,Y)$ from $Next$\;
				\If {($X$,$Y) \in c(Processed \cup Next)$}
				{skip\;}
				\If {$\neg(\{x\in X| x\in Q_\mathcal{A}\} \neq \varnothing \Leftrightarrow \{y\in Y| y\in Q_\mathcal{B}\}\neq \varnothing)$}
				{
					\Return FALSE\;
				}
				%\ForAll {$a\in \Sigma$}
				%{
					insert($post(X,X)$) in $Next$\;
				%}
...
The algorithm using bisimulation up to congruence for checking equivalence (and also inclusion)
of languages of NFA is described in its own section (\ref{sectionCongr}). This algorithm
reduces number of macro states that are necessary to check to determine if equivalence
holds or not thanks to the computation of a congruence closure of a relation
of the visited macro states because if the newly generated macro state is in the congruence closure it is not necessary to explore macro state which
...
An example (taken from \cite{popl13}) follows to illustrate an application of this approach for checking equivalency of 
...
Problem of this approach is that we do not know which rules of relation $R$ to use, in which order and each rule can be used only once for computing a normal
...
\label{congrOpt}
Since the algorithm based on bisimulation up to congruence is primarily used for checking equivalence of NFA it is possible to make some simplification for
checking inclusion. Some simplifications are possible also in checking whether macrostate $(X,Y)$ is in congruence closure of a relation $R$. This optimization
...
\end{center}
Usage of a rewriting rule in reverse direction ($X\rightarrow X\cup Y$) is not so effective because state of $\mathcal{B}$ are already in $X$ and because
during checking inclusion of two NFA it is not necessary to achieve $X{\downarrow_R}=Y{\downarrow_R}$ to prove that $(X,Y)\in c(R)$ 
