...
\chapter{Existing Finite Automata Libraries and the VATA Library} %TODO: zvazit zobecneni na prehled existujicich knihoven
\label{libraries}
...
%TODO dopsat computation ceho
The RWHT FSA toolkit supports on-demand computation for better memory efficiency. Another advantage is that it supports the weighted transitions 
so it is possible to use it
...
determinisation which are can be exploited for testing inclusion. There are used some optimization, but this not compensates the need of 
determinisation. 
...
The algorithm based on antichains and simulation introduced in \cite{taca10} have been implemented for tree automata in the VATA library. A description
...
There are supported basic operations like union, intersection, elimination of unreachable states, but also some advance a algorithms for inclusion checking, 
...
virtual method, because look-up in virtual-method precludes compiler's optimizer to perform some operations \cite{libvata}. %TODO dodelat lepsi citaci  Compiler Optimization of C++ Virtual Function Calls
...
automata will be definitely more efficient. This new extension will use some already implemented features of VATA like parsing and serializing
the input automata or computation of simulation over states of an automaton.

\chapter{Design}
\label{arch}
\section{Data Structure for Explicit FA Encoding}
\subsection{Simplyfing of the original data structure}
\subsection{Keeping the start states}
\subsection{Using timubk input format}
