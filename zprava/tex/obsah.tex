...
iff $p=r \wedge R \subseteq P$. The ordering $p\preceq r \wedge R\preceq^{\forall\exists}P$ is used for the optimization using simulation.
...
is not necessary in the case of basic version because where is checked just $p=r$.
...
causes unnecessary slowing down because the condition is always false. Although this optimization has not been already implemented, we suppose
that the separation of the functions of the both orderings will be also efficienty used in this case.

\subsection{Using Macrostate Cache}

\subsection{Ordered Antichain}
For checking inclusion of tree automata has not been used standard antichain for storing product states $(p,P)$ which should be further processed
but ordered antichain \cite{libvata} which preferres processing the elements with smaller size of macrostate $P$ first. This optimization lead to reduction
of produced states. The optimization has been implemented (using ordered antichain class of the original VATA library) also for checking language inclsion of
NFA and reduces the number of produced product states what brings the better performance to.
