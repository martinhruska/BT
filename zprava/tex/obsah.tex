...
automata will be given at first. Then the operations for basic operations like union or intersection will be explained and finally the algorithms (based
on bisimulation up-to congruence and on the antichain is given) for testing inclusion of the finite automata is given.

\section{Loading and Manipulation with an Finite Automata to the Explicit Encoding}
...
In this section are described algorithms used for implementation of basic like union, intersection or removing useless states and others. 
\subsection{Union}
...
The reindexing of states is done because the same integer can represent one state of NFA $\mathcal{A}$ and also another state of $\mathcal{B}$ and it is important
...
which is created just by assigning $I_\mathcal{A}$ to $F_{\mathcal{A}_{rev}}$ and $F_\mathcal{A}$ to $I_{\mathcal{A}_{rev}}$ what
is just changing start state's set by final state's set and reverting all transitions, so e.g., transition
$p\xrightarrow{x}q \in \delta_\mathcal{A}$ is added to $\delta_\mathcal{B}$ in form $q\xrightarrow{a}p$.
...
The intuition behind the algorithm is that the NFA $\mathcal{A}$ is explored from its start states and to the result automaton are added only states which are
reachable from this start states for some word $w \in \Sigma^{*}$. So firstly, all reachable states are found and then for each of them are
added to the result NFA $\mathcal{B}$ the transitions where is a reachable state on the left side of the transition. If a reachable state is final in 
$\mathcal{A}$, it is added also to set of final states in $\mathcal{B}$. A set of start states is coppied from
...
\begin{algorithm}[H]
  ...
  \ForAll{$p \in\ Reachable$}{
	...
	\caption{Algorithm for removing unreachable states of NFA}
\end{algorithm}
