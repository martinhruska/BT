...
VATA::ExplicitTreeAut<SymbolType> VATA::Intersection(
	...
	for (auto& s : lhs.finalStates_) {
      ...
      // The new pair, which is created from pair of states 
      // and its value is current size of translation map
			...
			stack.push_back(&*u); // iterator -> hodnota -> reference
	...
	while (!stack.empty()) {
		...
		for (auto& leftSymbolStateTupleSetPtr : *leftCluster) {
			...
			auto rightTupleSet = // Najdu spolecne stavy
				ExplicitTA::genericLookup(*rightCluster, leftSymbolStateTupleSetPtr.first);

			if (!rightTupleSet)
				continue;

			if (!cluster) // Haven't been already allocated
