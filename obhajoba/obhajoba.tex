...
\frame{\frametitle{Motivace}
		...
		\textcolor{Green}{Cíl}
		\begin{itemize}
			\item Vytvořit rozšíření knihovny \textcolor{red}{VATA} 
      pro práci s~konečnými automaty implementující nové a efektivní algoritmy.
...
\frame{\frametitle{Definice NKA}
    ...
    \begin{center}
  \begin{figure}
      \includegraphics<1>[scale=0.3]{tf_model.pdf}
	\end{figure}
    \end{center}
}
...
\frame{\frametitle{Použité algoritmy}
  \begin{itemize}
    \item \textcolor{Green}{Klasický přístup} k~testu inkluze $L(A)\cap \overline{L(B)}=\emptyset$
    \begin{itemize}
      \item Nutná determinizace $\Rightarrow$ stavová exploze
    \end{itemize}
    \item \textcolor{Green}{Algoritmus založený na bisimulaci vzhůru ke kongruenci}
    \item \textcolor{Green}{Antichains algoritmus}
 \end{itemize}
}
...
\frame{\frametitle{Porovnání s~OCaml verzí}
  ...
  \begin{center}
  \begin{figure}
%    \includegraphics<1>[scale=0.35]{plot_hkc_zprava.png}
    \includegraphics[scale=0.35]{plot_hkc_step_zprava.png}
  \end{figure}
   \end{center}
}

\frame{\frametitle{Porovnání obou algoritmů}

  \begin{center}
  \begin{figure}
    \includegraphics[width=0.5\textwidth]{plot_nfa_ac_zprava.png}
    \includegraphics[width=0.5\textwidth]{plot_nfa_congr_zprava.png}
  \end{figure}
  \end{center}
}
...
\frame{\frametitle{Závěr}
  \begin{itemize}
    \item Implementace rychlejší v~$>$92\% případů
    \item Další vývoj
    \begin{itemize}
      \item Plné využití simulace
    \end{itemize}
    \item 2. místo ve své kategorii na EEICT
  \end{itemize}
}
...
%\section{Otazky oponenta}
\frame{\frametitle{Otázka č.1}
\addtocounter{framenumber}{-1}
  \textcolor{Green}{Je algoritmus 5 převzatý, nebo jste ho navrhnul sám?}
  \begin{itemize}
    \item Algoritmus vytvoří pro daný automat $A$ jiný automat $B$, který přijímá
      nejkratší slovo jazyka $L_A$
    \item Vznikl modifikací téhož algoritmu pro stromové automaty
  \end{itemize}
}

\frame{\frametitle{Otázka č.2}
\addtocounter{framenumber}{-1}
  \textcolor{Green}{Mohli být vaše testy ovlivněny aktivitou ostatních uživatelů serveru merlin.fit.vutbr.cz?}
  \begin{itemize}
    \item Měřen čas na CPU
  \end{itemize}
}
%\end{comment}
...
\begin{center}
	\scalebox{0.7}
	{
		\includegraphics[scale=0.5]{congr1.pdf}
		\hspace{0.55cm}
  	\includegraphics[scale=0.5]{ac1.pdf}
	}
  \label{automata}
\end{center}
\end{figure}	
...
\frame{\frametitle{Datové struktury}
	\begin{itemize}
    \item Datová struktura pro ukládání přechodů KA v~\textcolor{red}{explicitním} kódování
	\end{itemize}
  \begin{center}
  \begin{figure}[h]
...
%\section{Algoritmus výpočtu kongruenčního uzávěru}
\frame{\frametitle{Algoritmus výpočtu kongruenčního uzávěru}
  \begin{itemize}
    \item $L_A\subseteq L_B \Leftrightarrow L_A\cup L_B = L_B$
    \item Mějme relaci navštívených stavů $R=\{(x,u),(y+z,u)\}$ a nově vygenerovaný stav $(\textcolor{Green}{x+y},\textcolor{blue}{u})$,
      kde $\textcolor{Green}{x,y}\in Q_A,\textcolor{blue}{u}\in Q_B$
  \end{itemize}
...
\frame{\frametitle{Algoritmus výpočtu kongruenčního uzávěru}
  \begin{itemize}
    \item $L_A\subseteq L_B \Leftrightarrow L_A\cup L_B = L_B$
    \item Mějme relaci navštívených stavů $R=\{(x+u,u),(y+z+u,u)\}$ a nově vygenerovaný stav $(\textcolor{Green}{x+y+u},
        \textcolor{blue}{u})$, kde $\textcolor{Green}{x,y}\in Q_A,\textcolor{blue}{u}\in Q_B$
  \end{itemize}
...
%\section{Antichain algoritmus}
\frame{\frametitle{Antichain algoritmus}
  \begin{itemize}
  \item Pro $(p,P)$ a $(q,Q)$ ověřuje $p\preceq q \wedge Q\preceq^{\forall\exists}P$, kde $p,q \in Q_A,P,Q \subseteq Q_B$
  \end{itemize}

  \textcolor{Green}{Optimalizace}
  \begin{itemize}
    \item Cache makrostavů
    \item Uspořádání na antichainu podle velikosti makrostavu
    \item Rozdělení funkcí ověřujicí relace uspořádání antichainu 
  \end{itemize}
}
...
%    \includegraphics<1>[scale=0.35]{plot_hkc_zprava.png}
  ...
  \vspace{0.5cm}
   ...
   \hline
    ...
    vítězí & $7.5\%$ & $92.5\%$ \\ \hline
    rychlejší & $6\times$ & $64\times$ \\ \hline
   \end{tabular}
 \end{center}
}

\frame{\frametitle{Porovnání kongruence s~VATA pro strom. automaty}

  \begin{center}
  \begin{figure}
    \includegraphics<1>[scale=0.35]{plot_ac_zprava.png}
    \includegraphics<2>[scale=0.35]{plot_ac_step_zprava.png}
  \end{figure}
  \vspace{0.5cm}
   \begin{tabular}{ | l | r | r |}
   \hline
    & \textbf{VATA AC SA $\uparrow$} & \textbf{VATA CONGR KA} \\ \hline \hline
    vítězí & $5\%$ & $95\%$ \\ \hline
    rychlejší & $1.27\times$ & $2.34\times$ \\ \hline
   \end{tabular}
 \end{center}
}
...
\frame{\frametitle{Porovnání kongruence s~VATA pro strom. automaty}
  ...
  \begin{center}
  \begin{figure}
    \includegraphics<1>[scale=0.35]{plot_ca_zprava.png}
    \includegraphics<2>[scale=0.35]{plot_ca_step_zprava.png}
  \end{figure}
  \vspace{0.5cm}
   \begin{tabular}{ | l | r | r |}
   \hline
    & \textbf{VATA AC $\downarrow$ KA} & \textbf{VATA CONGR KA} \\ \hline \hline
    vítězí & $6\%$ & $94\%$ \\ \hline
    rychlejší & $1.90\times$ & $161\times$ \\ \hline
   \end{tabular}
 \end{center}
}  

\frame{\frametitle{Porovnání antichain s~VATA pro strom. automaty}

  \begin{center}
  \begin{figure}
    \includegraphics<1>[scale=0.35]{plot_aca_zprava.png}
    \includegraphics<2>[scale=0.35]{plot_aca_step_zprava.png}
  \end{figure}
  \vspace{0.5cm}
   \begin{tabular}{ | l | r | r |}
   \hline
    & \textbf{VATA AC SA $\downarrow$} & \textbf{VATA AC KA} \\ \hline \hline
    vítězí & $7\%$ & $93\%$ \\ \hline
    rychlejší & $10.78\times$ & $211\times$ \\ \hline
   \end{tabular}
 \end{center}
}
...
\frame{\frametitle{Porovnání antichain s~VATA pro strom. automaty}
  ...
  \begin{center}
  \begin{figure}
    \includegraphics<1>[scale=0.35]{plot_aac_zprava.png}
    %\includegraphics<2>[scale=0.35]{plot_aac_step_zprava.png}
  \end{figure}
  \vspace{0.5cm}
   \begin{tabular}{ | l | r | r |}
   \hline
    & \textbf{VATA AC SA $\uparrow$} & \textbf{VATA AC KA} \\ \hline \hline
    vítězí & $2\%$ & $98\%$ \\ \hline
    rychlejší & $16\times$ & $2.7\times$ \\ \hline
   \end{tabular}
 \end{center}
}
...
\frame{\frametitle{Porovnání obou algoritmů}
  ...
  \vspace{0.5cm}
   ...
   \hline
    ...
    vítězí & $76\%$ & $24\%$ \\ \hline
    rychlejší & $1.58\times$ & $3.74\times$ \\ \hline
   \end{tabular}
 \end{center}
}
...
\setbeamertemplate{footline}{}
%\section{Otazky oponenta}
\frame{\frametitle{Otázky oponenta}
\addtocounter{framenumber}{-1}
   \begin{itemize}
  \item \textcolor{Green}{Algoritmus 5}
  \begin{itemize}
    \item Algoritmus vytvoří pro daný automat $A$ jiný automat $B$, který přijímá
      nejkratší slovo jazyka $L_A$
    \item Vznikl modifikací téhož algoritmu pro stromové automaty
  ...
  \item \textcolor{Green}{Testování na serveru merlin.fit.vutbr.cz}
  \begin{itemize}
    \item Testováno v~době mezi 1:00 a 7:00
    \item Testy na dvojici automatů pouštěny těsně po sobě
    \item Testováno i na dalším počítači
  \end{itemize}
